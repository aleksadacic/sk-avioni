...
public class UserController {
	...
	@Autowired
	public UserController(BCryptPasswordEncoder encoder, UserRepository userRepo, RankRepository rankRepo, KarticaRepository karticaRepo, EmailServiceImpl esi) {
	...
	public ResponseEntity<String> register(@RequestBody RegistrationForm registrationForm) {
		...
		try {
			...
			User user = new User(registrationForm.getIme(), registrationForm.getPrezime(), registrationForm.getEmail(),
					encoder.encode(registrationForm.getPassword()), registrationForm.getPasos(), rank);		
	...
	public ResponseEntity<String> editProfile(@RequestHeader(value = HEADER_STRING) String token, @RequestBody RegistrationForm registrationForm) {
		if (!CustomValidation.validateUser(registrationForm))
			return new ResponseEntity<String>("invalid parameters", HttpStatus.BAD_REQUEST);
		...
		try {
			...
			User user = userRepo.findByEmail(stari);
			if (registrationForm.getEmail().equalsIgnoreCase(stari) || (userRepo.findByEmail(registrationForm.getEmail()) != null && !userRepo.findByEmail(registrationForm.getEmail()).getEmail().equalsIgnoreCase(stari)))
				return new ResponseEntity<String>("email exists", HttpStatus.BAD_REQUEST);

			if (!registrationForm.getEmail().equalsIgnoreCase(stari))
				mailchanged = true;
			
			// iz frontenda saljemo podatke tako da su stare vrednosti popujnene u formi pa mogu da se edituju
	...
	public ResponseEntity<String> dodajKarticu(@RequestHeader(value = HEADER_STRING) String token, @RequestBody KarticaForm karticaForm) {
		try {
			...
			KreditnaKartica kartica = new KreditnaKartica(karticaForm.getIme(), karticaForm.getPrezime(), karticaForm.getBrojKartice(), karticaForm.getSigurnosniBroj());
	...
	public ResponseEntity<String> updateRank(@RequestHeader(value = HEADER_STRING) String token, @RequestBody RankForm rankForm) {
		try {
			if (!CustomValidation.validateRank(rankForm))
				throw new Exception();
			
			String stari = JWT.require(Algorithm.HMAC512(SECRET.getBytes())).build().verify(token.replace(TOKEN_PREFIX, "")).getSubject();
			User user = userRepo.findByEmail(stari);
	...
	public ResponseEntity<UserInfo> whoAmI(@RequestHeader(value = HEADER_STRING) String token) {
		try {
			...
			return new ResponseEntity<>(new UserInfo(user.getIme(), user.getPrezime(), user.getRankKorisnika()), HttpStatus.ACCEPTED);
		} catch (Exception e) {
			e.printStackTrace();
			return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
		}
	...
	public ResponseEntity<Object> pretragaLetova(@PathVariable String query) {
		try {
			//KORISNIK
			ResponseEntity<Object> response = UtilsMethods.sendGet("http://localhost:8081/letovi/pretraga/" + query, null);
			return response;
		} catch (Exception e) {
			e.printStackTrace();
			return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
